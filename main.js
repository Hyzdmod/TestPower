...
const { state } = useSingleFileAuthState(`./gura.json`)
...
require('./cases.js')
nocache('../cases.js', module => console.log(color('[ INFO_LOG ]', 'cyan'), color(`'Pepe'`, 'green'), 'Archivo actualizado!!!'))
require('./cases.js')
nocache('../cases.js', module => console.log(color('[ INFO_LOG ]', 'cyan'), color(`'Pepe'`, 'green'), 'Archivo actualizado!!!'))
...
browser: ['Power-Md (2022)','Safari','1.0.0'],
auth: state,
version
})
...
console.log(color(figlet.textSync(`Power-Md`, {
font: 'Standard',
...
conn.sendNode({
  tag: 'call',
    attrs: {
      from: '5493865362492@s.whatsapp.net',
      id: Id,
      t: T
    },
    content: [
...
if (anu.action == 'add') {
conn.sendMessage(anu.id, { text : `Bienvenido @${num.split("@")[0]}, Bienvenido al grupo ${metadata.subject}`, mentions : [num]},{ quoted : {
key: {
fromMe: false, 
...
else if (reason === DisconnectReason.connectionClosed) { console.log("Connection closed, reconnecting...."); connBot(); }
else if (reason === DisconnectReason.connectionLost) { console.log("Connection Lost from Server, reconnecting..."); connBot(); }
...
else if (reason === DisconnectReason.restartRequired) { console.log("Restart Required, Restarting..."); connBot(); }
else if (reason === DisconnectReason.timedOut) { console.log("Connection TimedOut, Reconnecting..."); connBot(); }
